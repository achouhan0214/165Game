...
public class GameObject
{
	...
	/** sets the rotation of the GameObject left or right */
	public void yaw(float angle)
	{
		float angleCos = (float)Math.cos(angle);
		float angleSin = (float)Math.sin(angle);
		Matrix4f rotMat = new Matrix4f(angleCos, 0, angleSin, 0, 0, 1, 0, 0, -angleSin, 0, angleCos, 0, 0, 0, 0, 1);
		Matrix4f tempWorldRot = this.getWorldRotation();
		this.setLocalRotation(tempWorldRot.mul(rotMat));
	}
	/** sets the rotation of the GameObject up or down */
	public void pitch(float angle)
	{
		float angleCos = (float)Math.cos(angle);
		float angleSin = (float)Math.sin(angle);
		Matrix4f rotMat = new Matrix4f(1, 0, 0, 0, 0, angleCos, -angleSin, 0, 0, angleSin, angleCos, 0, 0, 0, 0, 1);
		Matrix4f tempLocalRot = this.getLocalRotation();
		this.setLocalRotation(tempLocalRot.mul(rotMat));
	}
